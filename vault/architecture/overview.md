...
- `events_log`: `regex_matched_level`, `matched_status`, `matched_location`, `resolved_by` (`'lexicon'` | `'channel_state'` | `'channel_state_ambiguous'` — див. Рівень 2 нижче) — пишуться в `app/userbot/handlers.py::_store_event`
...
- `app/common/channel_state.py::resolve()` — єдина точка входу, приймає `DecisionTrace` від Рівня 1 (`lexicon.analyze()`) і `source_message_id`:
  ...
  - Рівно 1 активний слот → успадковує його рівень, оновлює локацію, якщо в повідомленні з'явився новий топонім; `layer='channel_state'`
...
- Redis-модель: `channel_state:{channel_id}:{slot}` (slot 0-2, до 3 одночасних цілей на канал), TTL 1200с (та сама константа "локаційно втраченої цілі", ТЗ §10), sliding — оновлюється при кожному успадкуванні
- Вибір слота при явному сигналі: спершу існуючий слот з тою самою локацією (та сама ціль, що розвивається), інакше вільний, інакше витісняється найстаріший
