...
- `app/common/llm.py` (НОВИЙ, стаб) — `compose_alert_text()` (текст сповіщення з структурованих даних, детермінований шаблон, `is_stub=True`, `composed_by='template_stub'` у БД) і `resolve_ambiguous_slot()` (винесений з `channel_state.resolve()` inline tie-break "найновіший слот" — поведінка не змінилась, зʼявився реальний шов під майбутню LLM). Жодного реального API-виклику/ключа — бюджетне рішення ADR-0012 не переглядається
