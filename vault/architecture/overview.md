...
**LLM-дальність для топонімів поза газетиром** (ADR-0012 "Уточнення 2026-07-24 (3)") — коли Рівень 1 не зловив ЖОДНОЇ локації (не плутати з tier=source — той випадок вище), `threat_state.record_signal()` питає `llm.judge_location_relevance(raw_text)` (четверта роль Рівня 3, fail-OPEN — недоступність/невпевненість ніколи не гасить сигнал). Кеш — Redis `geo_relevance:{назва}`, без TTL (дальність — статичний факт про назву, не про повідомлення); перед викликом LLM спершу `SCAN` уже вивчених назв і підрядкова перевірка, щоб не питати LLM про той самий топонім двічі.
