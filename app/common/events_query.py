...
_QUERY = (
    ...
    "    e.source_channel, e.reply_to_message_id, e.media_label, "
...
_CHAIN_LEVEL_QUERY = (
    "SELECT * FROM ("
    "  SELECT DISTINCT ON (e.source_channel, e.telegram_message_id) "
    "    e.source_channel, e.telegram_message_id, e.raw_text, e.reply_to_message_id, e.media_label, "
...
def _word_diff_html(old_text: str | None, new_text: str, media_label: str | None = None) -> Markup:
    ...
    Порожній текст — media_label ("📷 фото" тощо), якщо повідомлення без
    тексту було медіа, інакше голе "—" (справді порожнє повідомлення)."""
    new_text = new_text or ""
    empty_placeholder = escape(media_label) if media_label else escape("—")
    if old_text is None:
...
def _parse_history(raw_history, media_label: str | None = None) -> list[dict]:
    ...
    media_label — той самий для всіх версій: медіа не змінюється редагуванням
    підпису до нього, лише текст."""
    ...
    for version in parsed:
        version["diff_html"] = _word_diff_html(prev_text, version["raw_text"] or "", media_label)
...
async def load_recent_events(pool, limit: int) -> list[dict]:
    ...
    for event in events:
        ...
        versions = _parse_history(event["history"], event.get("media_label"))
        ...
        empty_placeholder = escape(event["media_label"]) if event.get("media_label") else escape("—")
